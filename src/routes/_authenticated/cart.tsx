...
function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F9E6D8] pt-6 md:pt-24 pb-24 md:pb-10 font-sans">
      <main className="max-w-4xl mx-auto px-4">
        ...
        {cartRows.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center shadow-sm border border-orange-100">
            ...
            <h2 className="text-xl font-bold text-[#4A2600] mb-2">ตะกร้าของคุณยังว่างอยู่</h2>
            <p className="text-gray-500 mb-8">ออกไปเลือกซื้อสินค้าดีๆ ให้สัตว์เลี้ยงของคุณกันเถอะ!</p>
            <button
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-orange-100 overflow-hidden">
              <div className="p-6 divide-y divide-gray-100">
                {cartRows.map((row) => (
                  <div key={row.id} className="py-6 first:pt-0 last:pb-0 flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between text-center sm:text-left">
                      <div>
                        <h3 className="font-bold text-lg text-[#4A2600] truncate">{row.name}</h3>
                        <p className="text-orange-600 font-black">฿ {row.price.toFixed(2)}</p>
                      </div>
                      <div className="flex items-center justify-center sm:justify-start gap-4 mt-4 sm:mt-0">
                        <div className="flex items-center bg-gray-100 rounded-xl p-1">
                          ...
                          <span className="w-10 text-center font-bold text-[#4A2600]">{row.qty}</span>
                          <button
                            onClick={() => setQuantity(row.id, row.qty + 1)}
                            className="w-8 h-8 flex items-center justify-center font-black text-[#4A2600] hover:bg-white rounded-lg transition-colors"
                          >
                    ...
                    <div className="text-center sm:text-right flex flex-col justify-center shrink-0">
                      ...
                      <p className="text-xl font-black text-[#4A2600]">฿ {row.subtotal.toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-[#4A2600] rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-end mb-6">
                  <div>
                    <p className="text-orange-200/70 text-sm font-bold uppercase tracking-widest mb-1">Total Items</p>
                    <p className="text-2xl font-black">{totalItems} ชิ้น</p>
                  </div>
                  <div className="text-right">
                    <p className="text-orange-200/70 text-sm font-bold uppercase tracking-widest mb-1">Grand Total</p>
                    <p className="text-4xl font-black text-[#FF914D]">฿ {totalPrice.toFixed(2)}</p>
                  </div>
                </div>
