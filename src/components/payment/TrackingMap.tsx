...
export function TrackingMap({ pickup, destination, freelancer }: TrackingMapProps) {
  const points: [number, number][] = [];
  if (pickup) points.push([pickup.lat, pickup.lng]);
  if (destination) points.push([destination.lat, destination.lng]);
  if (freelancer) points.push([freelancer.lat, freelancer.lng]);

  const center: [number, number] = points.length > 0 ? points[0] : [13.7563, 100.5018];
  ...
  return (
    ...
    >
      ...
      {freelancer && (
        <Marker position={[freelancer.lat, freelancer.lng]} icon={freelancerIcon}>
          <Popup>
            <div className="text-center">
              <p className="font-black text-[#A03F00] uppercase text-[10px]">Real-time Position</p>
              <p className="text-xs font-bold text-gray-500">Freelancer's location</p>
            </div>
          </Popup>
        </Marker>
      )}
