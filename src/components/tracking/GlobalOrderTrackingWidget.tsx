...
function GlobalOrderTrackingWidget() {
		...
		async (force = false) => {
			...
			try {
				...
				const productIds = Array.from(
					new Set(filtered.map((r) => r.product_id).filter(Boolean)),
				);
				const serviceIds = Array.from(
					new Set(filtered.map((r) => r.service_id).filter(Boolean)),
				);
				...
				(pRes.data || []).forEach((p: any) =>
					nMap.set(String(p.product_id), p.name),
				);
				(sRes.data || []).forEach((s: any) =>
					nMap.set(String(s.service_id), s.name),
				);

				const result = filtered.map((o: any) => ({
					id: String(o.order_id),
					name:
						nMap.get(String(o.product_id || o.service_id)) ||
						"Order " + String(o.order_id).slice(0, 4),
	...
	const loadTracking = useCallback(
		async (orderId: string, background = false) => {
			...
			try {
				...
				const [addrRes, prodRes, servRes] = await Promise.all([
					supabase
						...
						.in(
							"id",
							[
								orderRow.pickup_address_id,
								orderRow.destination_address_id,
							].filter(Boolean),
						),
					orderRow.product_id
						? supabase
				...
				const aMap = new Map(
					(addrRes.data || []).map((r: any) => [String(r.id), r]),
				);
				...
				setTracking({
					...
					destinationAddress:
						aMap.get(String(orderRow.destination_address_id)) || null,
	...
	return (
		<aside
			className={`fixed right-4 md:right-6 z-70 flex flex-col items-end pointer-events-none transition-all duration-300 ${isCheckoutFooterPage ? "bottom-25 md:bottom-6" : "bottom-4"}`}
		>
			{open && (
				<div className="mb-3 w-80 rounded-3xl border border-orange-100 bg-white/95 backdrop-blur-md text-[#4A2600] shadow-2xl overflow-hidden pointer-events-auto flex flex-col animate-in slide-in-from-bottom-4 duration-300">
					<div className="px-5 py-4 bg-[#4A2600] flex items-center justify-between text-white">
						...
						>
							<p className="text-[9px] font-black uppercase tracking-[0.2em] text-orange-200/60">
								Live Tracking
							</p>
							<p className="text-sm font-black truncate">
								{viewMode === "list"
									? `${ongoingOrders.length} jobs`
									: tracking?.productName || "Detail"}
							</p>
						</div>
							...
							<div className="max-h-60 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
								{ongoingOrders.map((o) => (
									<button
										...
										className={`w-full text-left p-3 rounded-2xl border transition-all ${o.id === activeOrderId ? "bg-orange-50 border-orange-200 shadow-sm" : "bg-white border-gray-100"}`}
									>
										<p className="text-[10px] font-black text-[#4A2600] truncate">
											{o.name}
										</p>
										<p className="text-[8px] font-bold text-orange-600 uppercase mt-0.5">
									...
									<>
										<div className="relative pt-2 pb-4">
											<div className="flex justify-between relative z-10 px-1">
												{STATUS_STEPS.map((step, idx) => {
													...
													return (
														<div
															key={step.key}
															className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${isActive ? "bg-[#A03F00] border-[#A03F00] text-white shadow-md" : "bg-white border-orange-50 text-gray-200"}`}
														>
											...
											<div className="absolute top-5.5 left-4 right-4 h-0.5 bg-orange-50 z-0">
												<div
													className="h-full bg-[#A03F00] transition-all duration-700"
													style={{
										...
										<div className="space-y-3 bg-orange-50/30 p-4 rounded-2xl border border-orange-100">
											<div className="flex justify-between items-center gap-2">
												<p className="text-[10px] font-black text-[#4A2600] uppercase tracking-widest">
													{tracking?.status.replace(/_/g, " ")}
												</p>
												<p className="text-[11px] font-black text-[#A03F00]">
													฿{tracking?.price.toLocaleString()}
												</p>
											</div>
										...
										<div className="flex gap-2">
											<Link
												to={`/chat/${tracking?.roomId}` as any}
												className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-orange-50 text-[#A03F00] border border-orange-100 font-black text-[9px] uppercase tracking-widest hover:bg-orange-100 transition-all"
											>
												<MessageCircle className="w-3.5 h-3.5" /> Chat
											</Link>
											<button
												onClick={openOrderPage}
												className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-[#A03F00] text-white font-black text-[9px] uppercase tracking-widest shadow-lg hover:bg-orange-800 transition-all"
											>
			...
			<button
				...
				className={`pointer-events-auto w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all shadow-2xl relative ${open ? "bg-white text-[#4A2600] border-2 border-orange-100" : "bg-[#A03F00] text-white"}`}
			>
