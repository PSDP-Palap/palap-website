...
function MapInteractionHandler({ onChange, readOnly }: { onChange: (lat: number, lng: number) => void; readOnly?: boolean; }) {
  useMapEvents({
    click: (e) => {
...
const MapPicker = ({ lat, lng, onChange, address, readOnly = false }: MapPickerProps) => {
  ...
  const currentPos: [number, number] = hasCoords ? [lat as number, lng as number] : DEFAULT_CENTER;
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden border border-gray-100 shadow-inner z-0 bg-orange-50 relative group">
      {!readOnly && (
        <>
          <div className="absolute top-4 left-4 right-4 z-[1000]">
            <form onSubmit={handleSearch} className="relative shadow-2xl rounded-2xl overflow-hidden">
              <input
                type="text"
                value={searchQuery}
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                {searchQuery && (
                  <button type="button" onClick={() => setSearchQuery("")} className="p-2 hover:bg-orange-50 rounded-xl transition-colors">
                    <X className="w-4 h-4 text-orange-400" />
                  </button>
                )}
                <button type="submit" disabled={isSearching} className="bg-orange-600 text-white p-2 rounded-xl hover:bg-orange-700 transition-all shadow-lg active:scale-95 disabled:opacity-50">
          ...
          <div className="absolute right-4 bottom-10 z-[1000]">
            <button onClick={handleGeolocation} disabled={isLocating} className="p-4 bg-white/95 backdrop-blur-md text-orange-600 rounded-2xl shadow-2xl hover:bg-orange-50 transition-all active:scale-95 border border-orange-100 disabled:opacity-50">
      ...
      <MapContainer center={currentPos} zoom={15} scrollWheelZoom={true} dragging={!readOnly} zoomControl={false} style={{ height: "100%", width: "100%" }}>
      ...
      {hasCoords && address && (
        <div className="absolute bottom-4 left-4 right-16 z-[1000] bg-white/95 backdrop-blur-md p-3 rounded-2xl border border-orange-100 shadow-xl pointer-events-none flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2">
          ...
          <div className="min-w-0">
            <p className="text-[9px] font-black text-orange-400 uppercase tracking-widest mb-0.5">Current Selection</p>
            <p className="text-xs font-bold text-orange-950 leading-relaxed line-clamp-2">{address}</p>
          </div>
        </div>
      )}
