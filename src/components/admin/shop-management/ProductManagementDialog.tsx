...
interface ProductManagementDialogProps {
  ...
  onClose: () => void;
  onUpdate: (id: string, data: Omit<Product, "product_id" | "created_at" | "pickup_address">) => Promise<void>;
...
export const ProductManagementDialog = ({
  ...
  const { uploadImage, addresses, loadAddresses, createAddress } = useProductStore();
  ...
  const [form, setForm] = useState<Omit<Product, "product_id" | "created_at" | "pickup_address">>({
    name: product?.name || "",
        ...
        <div className="p-8">
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div 
                ...
                className={`relative w-full h-56 rounded-2xl border-2 border-dashed transition-all flex items-center justify-center overflow-hidden ${
                  isEditing ? "cursor-pointer group hover:border-orange-300 bg-gray-50 hover:bg-orange-50/30" : "border-gray-100"
                }`}
              >
                {previewUrl ? (
                  <>
                  ...
                  <div className="flex flex-col items-center gap-2 text-gray-300">
                    ...
                    {isEditing && <span className="text-sm font-bold text-gray-400">คลิกเพื่ออัพโหลด</span>}
                  </div>
                )}
            ...
            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
              ...
              <p className="text-xs font-mono text-gray-500 break-all">{product.product_id}</p>
            </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="border-b border-gray-50 pb-2">
                    ...
                    <p className="font-black text-[#A6411C] text-lg">{product.price.toLocaleString()} ฿</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="border-b border-gray-50 pb-2">
                    ...
                    <p className="font-bold text-gray-800">{product.qty.toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 text-left">
                  ...
                  {product.pickup_address ? (
                    <div>
                      <p className="text-sm font-bold text-gray-800">{product.pickup_address.name}</p>
                      <p className="text-xs text-gray-500">{product.pickup_address.address_detail || "-"}</p>
                    </div>
                  ) : (
                    <p className="text-sm text-gray-400">ไม่ได้ระบุที่อยู่นัดรับ</p>
                  )}
                </div>

                <div className="pt-4 flex gap-3">
                  <button onClick={() => setIsEditing(true)} className="flex-1 bg-orange-600 text-white py-3 rounded-xl font-bold hover:bg-orange-700 transition-colors shadow-lg shadow-orange-100">
                    Edit Product
                  </button>
                  <button onClick={handleDelete} className="flex-1 bg-red-50 text-red-600 py-3 rounded-xl font-bold hover:bg-red-100 transition-colors">
                    Delete
                  </button>
                </div>
              </div>
              ...
              <form onSubmit={handleUpdate} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Name</label>
                    <input type="text" name="name" value={form.name} onChange={handleChange} className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" required />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Price (฿)</label>
                    <input type="number" name="price" value={form.price} onChange={handleChange} className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" required />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Quantity</label>
                    <input type="number" name="qty" value={form.qty} onChange={handleChange} className="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500" required />
                  </div>
                </div>
                ...
                <div className="space-y-3 pt-2 border-t border-gray-50 text-left">
                  ...
                  {!isAddingNewAddress ? (
                    <div className="space-y-3">
                      {form.pickup_address_id ? (
                        <div className="p-4 bg-orange-50/30 rounded-2xl border border-orange-100/50 flex justify-between items-center group text-left">
                          <div>
                            <p className="text-sm font-bold text-gray-800">
                              {product?.pickup_address?.name || "ที่อยู่ปัจจุบัน"}
                            </p>
                            ...
                            onClick={() => {
                              ...
                              if (product?.pickup_address) {
                                setNewAddressForm({
                                  ...
                                  address_detail: product.pickup_address.address_detail || "",
                        ...
                        >
                          ...
                          <span className="text-sm font-bold">ระบุที่อยู่นัดรับสินค้า</span>
                        </button>
                      )}
                    </div>
                  ) : (
                    <div className="space-y-3 p-4 bg-orange-50/30 rounded-2xl border border-orange-100/50 relative animate-in fade-in slide-in-from-top-2 duration-200 text-left">
                      ...
                      <input type="text" placeholder="ชื่อที่อยู่" value={newAddressForm.name} onChange={(e) => setNewAddressForm((prev) => ({ ...prev, name: e.target.value }))} className="w-full border border-white bg-white/80 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" />
                      <input type="text" placeholder="รายละเอียด" value={newAddressForm.address_detail} onChange={(e) => setNewAddressForm((prev) => ({ ...prev, address_detail: e.target.value }))} className="w-full border border-white bg-white/80 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" />
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">เลือกตำแหน่งบนแผนที่</label>
                        <MapPicker
                ...
                <div className="pt-4 flex gap-3">
                  <button type="submit" disabled={isSubmitting} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition-colors shadow-lg shadow-green-100 disabled:opacity-50">
                    {isSubmitting ? "กำลังบันทึก..." : "Save Changes"}
                  </button>
                  <button type="button" onClick={() => setIsEditing(false)} disabled={isSubmitting} className="flex-1 bg-gray-100 text-gray-600 py-3 rounded-xl font-bold hover:bg-gray-200 transition-all disabled:opacity-50">
                    Cancel
                  </button>
                </div>
              </form>
